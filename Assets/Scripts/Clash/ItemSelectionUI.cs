...
{
    public sealed class ItemSelectionUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        [Header("Panel")]
        ...
        [SerializeField] private ItemCarouselItem itemTemplate;
        ...
        private readonly List<ItemCarouselItem> items = new();
        private ClashItemConfig[] availableItems;
        private Action<ClashItemConfig> onSelected;
        private float scrollPosition;
        private bool dragging;

        private int CenterIndex => availableItems == null || availableItems.Length == 0
            ? 0
        ...
        {
            string[] legacyNames = { "Card_N", "Card_R", "Card_SR", "Item_N", "Item_R", "Item_SR" };
        ...
        private ItemCarouselItem CreateRuntimeItemTemplate(Transform parent)
        {
            GameObject itemObject = CreateRuntimeUiObject("RuntimeItemTemplate", parent, new Vector2(200f, 260f), new Vector2(0.5f, 0.5f), Vector2.zero);
            itemObject.AddComponent<CanvasGroup>();

            Image artworkImage = itemObject.AddComponent<Image>();
            artworkImage.preserveAspect = true;
            artworkImage.color = Color.white;

            GameObject clickArea = CreateRuntimeStretchObject("RuntimeItemClickArea", itemObject.transform);
            Image clickImage = clickArea.AddComponent<Image>();
            clickImage.color = new Color(1f, 1f, 1f, 0.001f);
            clickArea.AddComponent<Button>();

            ItemCarouselItem item = itemObject.AddComponent<ItemCarouselItem>();
            itemObject.SetActive(false);

            item.ConfigureView(artworkImage);
            return item;
        }
        ...
        public void Show(ClashItemConfig[] newAvailableItems, Action<ClashItemConfig> selectedCallback)
        {
        ...
        public void ConfirmSelection()
        {
            ...
            onSelected?.Invoke(availableItems[CenterIndex]);
        }

        private void RebuildItems()
        {
            foreach (ItemCarouselItem item in items)
            {
                if (item != null)
                {
            ...
            if (availableItems == null || itemTemplate == null || itemRoot == null)
            {
                return;
            }
            ...
            {
                ItemCarouselItem item = Instantiate(itemTemplate, itemRoot);
                item.name = $"Item_{i + 1:00}_{availableItems[i].Rarity}";
                item.gameObject.SetActive(true);
                item.SetItem(availableItems[i]);
        ...
        private void NormalizeScrollPosition()
        {
            ...
            if (Mathf.Abs(scrollPosition) <= availableItems.Length * 2f)
            {
                return;
            }

            int wholeLoops = Mathf.FloorToInt(scrollPosition / availableItems.Length);
            scrollPosition -= wholeLoops * availableItems.Length;
        }
        ...
        private int WrapIndex(int index)
        {
            ...
            int count = availableItems.Length;
